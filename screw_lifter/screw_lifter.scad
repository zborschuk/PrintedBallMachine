...
if(part == 0)
    screw_inlet();
if(part == 1)
    screw_segment(length=5.5, starts=2, top=ROUND);
if(part == 2)
    screw_outlet();
if(part == 3)
    screw_guide();
...
module assembled(){
    ...
    translate([peg_sep*2,screw_offset,0]) rotate([0,angle,90]) translate([0,0,0]) screw_segment(length=5.5, starts=2, top=ROUND);
...
module screw_inlet(){
    ...
    difference(){
        union(){
           ...
           //false floor
           mirror([0,1,o]) cube([in*3, in*3, in/2+1]);
           
           //top guide
           translate([in*1.25,in/2+screw_offset+in*.1,in*4]) scale([1,1.2,1]) track(rise = -in/4, run = in*3);
        }
        
        translate([in*1.5,in/2+screw_offset,in*4]) track(rise = -in/4, run = in*3, solid=-1);
        
        //guide track hollow
           translate([in*2,in/2+screw_offset,wall*2]) hull() for(i=[0,1]) mirror([i,0,0]) rotate([0,0,open_angle]) translate([screw_rad+ball_rad+wall*1.5,0,0]) rotate([0,-90,0]) track(rise = 0, run = in*4.5, solid=-1);
